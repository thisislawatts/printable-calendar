...
export function renderPdfDocument(calender, options: PdfDocumentSettings, settingsVector: string) {
	let doc: any;
	...
	calender.months.forEach(({ days, title }) => {
		...
		days.forEach((day: any, idx: number) => {
			...
			doc.text(day.dayOfMonth + '', x + CELL_PADDING, y - (ROW_HEIGHT - CELL_PADDING * 3));
		...
		for (let index = 1; index < calender.weekdayHeadings.length; index++) {
			...
			let numberOfRowsToLine = !itemsOnLastRow || index > itemsOnLastRow ? rowCount - 1 : rowCount;
			doc.line(x, TABLE_Y - HEADER_ROW_HEIGHT, x, TABLE_Y + ROW_HEIGHT * numberOfRowsToLine);
		}

		// Add attribution link
		doc.setFontSize(6);
		doc.text(`https://calendar.thisis.la/?${settingsVector}`, TABLE_X, SHEET_DIMENSIONS[1] - 2);
	});
